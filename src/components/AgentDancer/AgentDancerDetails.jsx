 ...
 setSelectedVideoIndex(index);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/api/agent-dancers/${id}?populate=*`
      );
      const responseData = response?.data?.data;
      setDetails(responseData);
      console.log(details,'Agent dancer details')
  ...
  const handleSubmit = async () => {
    try {
      ...
      message.success('Agent Dancer Request submitted successfully!');
      setModalVisible1(false);
      form.resetFields();
    } catch (error) {
      console.error(error);
      message.error('Failed to submit the Agent dancer request!');
    }
        ...
        {loading ? (
          ...
          <>
            <Backdrop>
              ...
              <Look><span> <StarOutlined /> AgentDancer_ID:{" "}</span>{details?.attributes?.agentDancer_ID || "Agent Dancer ID Does Not Exists"}</Look>
                  ...
                  {details?.attributes?.VideoFile?.data?.map((dancer) => (
                    <div key={dancer.id}
                      ...
                      //  style={{ height: '100%', width: '100%' }}
                        ...
                        className="DancerImage" controls autoPlay loop>
                    <source src={`${API_URL}${dancer?.attributes?.url}`} type="video/mp4" />
                    Your browser does not support the video tag.
                  </video>
                    </div>
...
export default AgentDancerDetails;
