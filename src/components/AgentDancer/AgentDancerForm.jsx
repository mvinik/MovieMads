 ...
 const [fullName, setFullName] = useState("");
  ...
  const handleInputChange = (e) => {
    ...
    switch (name) {
      ...
      case "danceStyle":
        ...
        break;
      case "danceSchool":
        setDanceSchool(value);
        break;
      case "performancePreference":
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      const response = await axios.post(`${API_URL}/api/agent-dancers`, {
        data: {
         Name: values.fullName,
          ...
          DanceStyle: values.danceStyles, // Note: matches form field name
          DanceSchool: values.danceSchool,
          PerformancePreference: values.performancePreference,
          NextSteps: Array.isArray(values.nextSteps) ? values.nextSteps.join(',') : values.nextSteps,
      ...
      // console.log(response,'Agent dancer res')
      const AgentDancerId = response.data.data.id;
      localStorage.setItem("AgentDancerId", AgentDancerId);
  ...
  const handleUpload = async () => {
    ...
    // console.log(localStorage.getItem('DancerId'),'DancerId Poster');
    for (let i = 0; i < imageFormDatas.length; i++) {
      const imageFormData = new FormData();
      imageFormData.append('files', imageFormDatas[i]);
      imageFormData.append('refId', localStorage.getItem("AgentDancerId"))
      imageFormData.append('ref', 'api::agent-dancer.agent-dancer')


      if (i == 0) {
        imageFormData.append('field', "Poster")
        const newFileData = {
          alternativeText: localStorage.getItem("AgentDancerId"),
          caption: 'Poster',
        };
    ...
    if (videoUpload) {
      ...
      videoFormData.append('refId', localStorage.getItem("AgentDancerId"))
      videoFormData.append('ref', 'api::agent-dancer.agent-dancer')
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (Paymentresponse) {
          ...
          try {
            const paymentResponse = await axios.post(`${API_URL}/api/agent-dancers/${localStorage.getItem('AgentDancerId')}/${Paymentresponse.razorpay_payment_id}/payment`, {}, option1);
            ...
            window.location.href = "/agentDancerForm";
          } catch (error) {
  ...
  return (
    <>
      ...
      <div className="container">
        <div>
          <h1 className='contest-heading'>Agent Dancer form <p style={{ fontSize: '1.5rem', padding: '0', margin: '0' }}>(Entry fee of <p className='strikeOut'>Rs.999</p> Now Rs.{discount > 0 ? (<>{finalPrice}</>) : (<>{price}</>)} only)</p> </h1>
        </div>
                  ...
                  <>
                    ...
                    <div className='Two input'>
                      ...
                      <Form.Item
                        label="Dance Style(s)"
                        name="danceStyles"
                        rules={[{ required: true, message: 'Please Enter Dance Style(s)!' }]}
                        className="input-container"
                      >
                        <Input placeholder="e.g., Hip Hop, Bharatanatyam" />
                      </Form.Item>
                    </div>
                    <div className='Two input'>
                      <Form.Item
                    ...
                    <div className='Two input'>
                      ...
                      <Form.Item
                        ...
                        rules={[{ required: true, message: 'Please select your performance preference!' }]}
                        className="input-container"
                      >
                        <Select>
                          <Select.Option value="Solo">Solo Performances</Select.Option>
                          <Select.Option value="Group">Group Performances</Select.Option>
                          <Select.Option value="Both">Both Solo and Group</Select.Option>
                        </Select>
                      </Form.Item>
                    </div>
                    <div className="Two input">
                      <Form.Item
                        label="Trained In / Dance School Name"
                        name="danceSchool"
                        rules={[{ required: true, message: 'Please enter your dance training background!' }]}
                        className="input-container"
                      >
                        <Input placeholder="e.g., Shiamak Davar Institute, Self-taught, etc." />
                      </Form.Item>


                      <Form.Item
                        label="What are you looking for next in your dance career?"
                        name="nextSteps"
                        rules={[{ required: true, message: 'Please tell us about your dance career aspirations!' }]}
                        className="input-container"
                      >
                        <Select
                          mode="tags"
                          ...
                          value={nextSteps}
                          tokenSeparators={[',']}
                          onChange={(values) => setNextSteps(values)}
                          onInputKeyDown={(e) => {
                            if (['Enter', ','].includes(e.key)) {
                              e.preventDefault();
                              const inputValue = e.target.value.trim();
                              if (inputValue && !nextSteps.includes(inputValue)) {
                                setNextSteps(prev => [...prev, inputValue]);
                                e.target.value = '';
                              }
                            }
                          }}
                          ...
                          showSearch
                        >
                          <Select.Option value="Performance Opportunities">Performance Opportunities</Select.Option>
                          <Select.Option value="Choreography Work">Choreography Work</Select.Option>
                          <Select.Option value="Teaching Opportunities">Teaching Opportunities</Select.Option>
                          <Select.Option value="Reality Show Participation">Reality Show Participation</Select.Option>
                          <Select.Option value="Film Industry Opportunities">Film Industry Opportunities</Select.Option>
                          <Select.Option value="Collaborations">Collaborations</Select.Option>
                        </Select>
                      </Form.Item>
                ...
                {currentStep === 2 && (
                  <div>
                    <div style={{ height: '500px', overflow: 'auto' }}>
                      <h2>Terms and Conditions for Dancer Application</h2>
                      <ol>
                        <li><strong style={{ color: '#ff0015' }}>Categories</strong>: The contest is open to three categories of dancers: male, female, and child artist.</li>
                        ...
                        <li><strong style={{ color: '#ff0015' }}>Selection Criteria</strong>: Selection and approval of profiles will be based on criteria set by the contest organizers, including but not limited to appearance, charisma, and suitability for dancing.</li>
                        ...
                        <li><strong style={{ color: '#ff0015' }}>Facilitation of Connections</strong>: The organizers will facilitate connections between dancers and interested parties such as directors or others who wish to hire dancer from the published list.</li>
                        <li><strong style={{ color: '#ff0015' }}>Dancer Release</strong>: Selected participants will be required to sign a dancer form, granting the organizers permission to use their likeness in promotional materials.</li>
...
export default AgentDancerForm;