 ...
 setSelectedVideoIndex(index);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/api/agent-musicians/${id}?populate=*`
      );
      const responseData = response?.data?.data;
      setDetails(responseData);
      console.log(details,'Agent Musician details')
  ...
  const handleSubmit = async () => {
    try {
      ...
      message.success('Agent Musician Request submitted successfully!');
      setModalVisible1(false);
      form.resetFields();
    } catch (error) {
      console.error(error);
      message.error('Failed to submit the Agemt SOM request!');
    }
  ...
  return (
        ...
        {loading ? (
          ...
          <>
            ...
            <DetailsContainer>
              ...
              <Look style={{ margin: "10px 0" }}>
                            <span>
                              <StarOutlined /> Primary Instrument:{" "}
                            </span>{" "}
                            {details?.attributes?.PrimaryInstrument}
                          </Look>
                          <Look style={{ margin: "10px 0" }}>
                            <span>
                              <StarOutlined /> Musical Genner:{" "}
                            </span>{" "}
                            {details?.attributes?.MusicalGeners}
                          </Look>
                            {/* <Look style={{ margin: "10px 0" }}>
                            <span>
                              <StarOutlined /> Performance Type:{" "}
                            </span>{" "}
                            {details?.attributes?.PerformanceType}
                          </Look> */}
                            <Look style={{ margin: "10px 0" }}>
                            <span>
                              <StarOutlined /> Formal Training:{" "}
                            </span>{" "}
                            {details?.attributes?.FormalTraining}
                          </Look>
                            <Look style={{ margin: "10px 0" }}>
                            <span>
                              <StarOutlined /> Can Read Music:{" "}
                            </span>{" "}
                            {details?.attributes?.CanReadMusic}
                          </Look>
                          <Look>
                            <span>
                              <StarOutlined /> Experience:{" "}
                            </span>
                            {details?.attributes?.Yearofexperience}{" "}
                          </Look>
                          <Look1><span> <StarOutlined /> AgentMusician_ID:{" "}</span>{details?.attributes?.agentMusician_ID || "Agent Musician ID Does Not Exists"}</Look1>
              {/* </div> */}
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <Form.Item
                      label="Your Message to our Musician"
                      name="messages"
   ...
   <h1 style={{ textAlign: "center" }}>
      ...
      <Container>
        <Content1>
          {details?.attributes?.Images?.data?.map((musician, index) => (
            <>
              <div key={musician.id}>
                <div className="movieTrailers-container1">
                  <img
                    src={`${API_URL}${musician?.attributes?.url}`}
              ...
              >
                <Carousel
                  ...
                  useKeyboardArrows
                >
                  {details?.attributes?.Images?.data?.map((musician) => (
                    <div key={musician.id}
                      style={{
                        display: 'flex',
                        justifyContent: 'center',
  ...
  <>
      ...
      <Container>
        <Content1>
          {details?.attributes?.VideoFile?.data?.map((musician, index) => (
            <>
              <div key={musician?.id}>
                <div className="movieTrailers-container1"
                ...
                >
                  <video style={{ height: '100%', width: '100%' }} controls autoPlay loop>
                    <source src={`${API_URL}${musician?.attributes?.url}`} type="video/mp4" />
              ...
              >
                <Carousel
                  ...
                  useKeyboardArrows
                >
                  {details?.attributes?.VideoFile?.data?.map((musician) => (
                    <div key={musician.id}
                      style={{
                        display: 'flex',
                        justifyContent: 'center',
                      ...
                      //  style={{ height: '100%', width: '100%' }}
                        ...
                        className="MusicianImage" controls autoPlay muted loop>
                    <source src={`${API_URL}${musician?.attributes?.url}`} type="video/mp4" />
                    Your browser does not support the video tag.
                  </video>
                    </div>
...
export default AgentMusicianDetails;
