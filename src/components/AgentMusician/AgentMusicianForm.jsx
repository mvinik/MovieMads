...
import { Steps, Form, Input, ConfigProvider, Button, Radio,Upload, Space, Progress, Select, notification } from 'antd';
...
const AgentMusicianForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      const response = await axios.post(`${API_URL}/api/agent-musicians`, {
        data: {
      ...
      // console.log(response,'Agent musician res')
      const AgentMusicianId = response.data.data.id;
      localStorage.setItem("AgentMusicianId", AgentMusicianId);
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < imageFormDatas.length; i++) {
      ...
      imageFormData.append('refId', localStorage.getItem("AgentMusicianId"))
      imageFormData.append('ref', 'api::agent-musician.agent-musician')


      if (i == 0) {
        imageFormData.append('field', "Poster")
        const newFileData = {
          alternativeText: localStorage.getItem("AgentMusicianId"),
          caption: 'Poster',
        };
    ...
    if (videoUpload) {
      ...
      videoFormData.append('refId', localStorage.getItem("AgentMusicianId"))
      videoFormData.append('ref', 'api::agent-musician.agent-musician')
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (Paymentresponse) {
          ...
          try {
            const paymentResponse = await axios.post(`${API_URL}/api/agent-musicians/${localStorage.getItem('AgentMusicianId')}/${Paymentresponse.razorpay_payment_id}/payment`, {}, option1);
            ...
            window.location.href = "/agentMusicianForm";
          } catch (error) {
  ...
  return (
    <>
      ...
      <div className="container">
        <div>
          <h1 className='contest-heading'>Agent Musician form <p style={{ fontSize: '1.5rem', padding: '0', margin: '0' }}>(Entry fee of <p className='strikeOut'>Rs.999</p> Now Rs.{discount > 0 ? (<>{finalPrice}</>) : (<>{price}</>)} only)</p> </h1>
        </div>
...
export default AgentMusicianForm;