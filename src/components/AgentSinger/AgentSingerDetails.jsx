 ...
 setSelectedVideoIndex(index);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/api/agent-singers/${id}?populate=*`
      );
      const responseData = response?.data?.data;
      setDetails(responseData);
      console.log(details,'Agent singer details')
  ...
  const handleSubmit = async () => {
    try {
      ...
      message.success('Agent Singer Request submitted successfully!');
      setModalVisible1(false);
      form.resetFields();
    } catch (error) {
      console.error(error);
  ...
  return (
        ...
        {loading ? (
          ...
          <>
            ...
            <DetailsContainer>
              ...
              <Look1><span> <StarOutlined /> Singer_ID:{" "}</span>{details?.attributes?.agentSinger_ID || "Singer ID Does Not Exists"}</Look1>
              {/* </div> */}
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <Form.Item
                      label="Your Message to our Singer"
                      name="messages"
   ...
   <h1 style={{ textAlign: "center" }}>
      ...
      <Container>
        <Content1>
          {details?.attributes?.Images?.data?.map((singer, index) => (
            <>
              <div key={singer.id}>
              ...
              >
                <Carousel
                  ...
                  useKeyboardArrows
                >
                  {details?.attributes?.Images?.data?.map((singer) => (
                    <div key={singer.id}
                      style={{
                        display: 'flex',
                        justifyContent: 'center',
  ...
  <>
      ...
      <Container>
        <Content1>
          {details?.attributes?.VideoFile?.data?.map((singer, index) => (
            <>
              <div key={singer?.id}>
                <div className="movieTrailers-container1"
                ...
                >
                  <video style={{ height: '100%', width: '100%' }} controls autoPlay muted  loop>
                    <source src={`${API_URL}${singer?.attributes?.url}`} type="video/mp4" />
              ...
              >
                <Carousel
                  ...
                  useKeyboardArrows
                >
                  {details?.attributes?.VideoFile?.data?.map((singer) => (
                    <div key={singer.id}
                      style={{
                        display: 'flex',
                        justifyContent: 'center',
...
export default AgentSingerDetails;
