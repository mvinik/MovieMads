...
const AgentSingerForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      const response = await axios.post(`${API_URL}/api/agent-singers`, {
        data: {
      ...
      // console.log(response,'Agent singer res')
      const AgentSingerId = response.data.data.id;
      localStorage.setItem("AgentSingerId", AgentSingerId);
  ...
  const handleUpload = async () => {
    ...
    // console.log(localStorage.getItem('SingerId'),'SingerId Poster');
    for (let i = 0; i < imageFormDatas.length; i++) {
      const imageFormData = new FormData();
      imageFormData.append('files', imageFormDatas[i]);
      imageFormData.append('refId', localStorage.getItem("AgentSingerId"))
      imageFormData.append('ref', 'api::agent-singer.agent-singer')


      if (i == 0) {
        imageFormData.append('field', "Poster")
        const newFileData = {
          alternativeText: localStorage.getItem("AgentSingerId"),
          caption: 'Poster',
        };
    ...
    if (videoUpload) {
      ...
      videoFormData.append('refId', localStorage.getItem("AgentSingerId"))
      videoFormData.append('ref', 'api::agent-singer.agent-singer')
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (Paymentresponse) {
          ...
          try {
            const paymentResponse = await axios.post(`${API_URL}/api/agent-singers/${localStorage.getItem('AgentSingerId')}/${Paymentresponse.razorpay_payment_id}/payment`, {}, option1);
            ...
            window.location.href = "/agentSingerForm";
          } catch (error) {
  ...
  return (
    <>
      ...
      <div className="container">
        <div>
          <h1 className='contest-heading'>Agent Singer form <p style={{ fontSize: '1.5rem', padding: '0', margin: '0' }}>(Entry fee of <p className='strikeOut'>Rs.999</p> Now Rs.{discount > 0 ? (<>{finalPrice}</>) : (<>{price}</>)} only)</p> </h1>
        </div>
                  ...
                  <>
                    ...
                    >
                      ...
                      <div className='Two input'>
                        ...
                        >
                          <TextArea rows={6}
                            placeholder='Discribe about your singing talent' />
                        </Form.Item>
                ...
                {currentStep === 2 && (
                  <div>
                    <div style={{ height: '500px', overflow: 'auto' }}>
                      <h2>Terms and Conditions for Singer Application</h2>
                      <ol>
                        <li><strong style={{ color: '#ff0015' }}>Categories</strong>: The contest is open to three categories of singers: male, female, and child artist.</li>
                        ...
                        <li><strong style={{ color: '#ff0015' }}>Selection Criteria</strong>: Selection and approval of profiles will be based on criteria set by the contest organizers, including but not limited to appearance, charisma, and suitability for singing.</li>
                        ...
                        <li><strong style={{ color: '#ff0015' }}>Facilitation of Connections</strong>: The organizers will facilitate connections between singers and interested parties such as directors or others who wish to hire singers from the published list.</li>
                        <li><strong style={{ color: '#ff0015' }}>Singer Release</strong>: Selected participants will be required to sign a singer release form, granting the organizers permission to use their likeness in promotional materials.</li>
...
export default AgentSingerForm;