...
import { Steps, Form, Input, ConfigProvider, Button, Upload, Progress, Select, notification, Space } from 'antd';
...
const DancerForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      const response = await axios.post(`${API_URL}/api/dancers`, {
        data: {
      ...
      const DancerId = response.data.data.id;
      localStorage.setItem("DancerId", DancerId);
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < imageFormDatas.length; i++) {
      ...
      imageFormData.append('refId', localStorage.getItem("DancerId"))
      imageFormData.append('ref', 'api::dancer.dancer')


      if (i == 0) {
        imageFormData.append('field', "Poster")
        const newFileData = {
          alternativeText: localStorage.getItem("DancerId"),
          caption: 'Poster',
        };
    ...
    if (videoUpload) {
      ...
      videoFormData.append('refId', localStorage.getItem("DancerId"))
      videoFormData.append('ref', 'api::dancer.dancer')
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (Paymentresponse) {
          ...
          try {
            const paymentResponse = await axios.post(`${API_URL}/api/dancers/${localStorage.getItem('DancerId')}/${Paymentresponse.razorpay_payment_id}/payment`, {}, option1);
  ...
  return (
    <>
      ...
      <div className="container">
        <div>
          <h1 className='contest-heading'>Dancer form <p style={{ fontSize: '1.5rem', padding: '0', margin: '0' }}>(Entry fee of <p className='strikeOut'>Rs.999</p> Now Rs.{discount > 0 ? (<>{finalPrice}</>) : (<>{price}</>)} only)</p> </h1>
        </div>
        {loading ? (
