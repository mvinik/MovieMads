...
const EditDancer = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setDanceSchool(responseData?.attributes?.DanceSchool);
      setDanceStyle(responseData?.attributes?.DanceStyle || "")
      setPerformancePreference(responseData?.attributes?.PerformancePreference)
      setNextSteps(responseData?.attributes?.NextSteps)
  ...
  const handleFieldChange = async () => {
    try {
      const res = await axios.put(`${API_URL}/api/Dancers/${id}`, {
        data: {
          Name: fullName,
  ...
  const handleImagesUpload = async (img) => {
    try {
      ...
      if (imageId) {
        ...
        newImageData.append('refId', localStorage.getItem('DancerId'));
        newImageData.append('ref', 'api::dancer.dancer');
        // Upload new image
        ...
        window.location.reload();
        // console.log(localStorage.getItem('SingerId'),res.data, 'new image upload response');
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const handleVideoFileUpload = async (video) => {
    ...
    try {
      ...
      const newFileData = {
        alternativeText: details?.attributes?.Name || "Dancer Video",
        caption: 'Dancer Video File',
      };
      formData.append('fileInfo', JSON.stringify(newFileData));
      ...
      // If adding new video
        ...
        formData.append('ref', 'api::dancer.dancer');
              ...
              <div className="profile-details">
                {!editing ? (
                  <>
                    ...
                    <p className="bio"><span>Dance Style: {details?.attributes?.DanceStyle}</span></p>
                    <p className="bio"><span>Performance Preference: {details?.attributes?.PerformancePreference}</span></p>
                    <p className="bio"><span>Next Step: {details?.attributes?.NextSteps}</span></p>
                    <p className="bio"><span>Trained In: {details?.attributes?.DanceSchool}</span></p>
              ...
              <div className='right-up'>
                <div className='r1'>
                  ...
                  <div className="gallery-item gallery1">
                    <img src={`${API_URL}${details?.attributes?.Poster?.data[0]?.attributes?.url}`} alt={`Singer Image`} className="gallery-image" />
                ...
                <div className='r1'>
                  ...
                  <div className="gallery-item gallery1">
                    <img src={`${API_URL}${details?.attributes?.Thumbnail?.data[0]?.attributes?.url}`} alt={`Singer Image`} className="gallery-image" />
                  ...
                  {[...Array(5)].map((_, index) => {
                    ...
                    return (
                      <div key={index} className="gallery-item">
                        {image ? (
                          <>
                            <img src={`${API_URL}${image?.attributes?.url}`} alt={`Singer ${index}`} className="gallery-image" />
                          ...
                          <div className="gallery-item">
                            <img src={`https://api.moviemads.com/uploads/empty_image_dd70f20899.jpg`} alt={`Singer ${index}`} className="gallery-image" />
