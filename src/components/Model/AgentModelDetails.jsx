  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/api/agent-models/${id}?populate=*`
      );
      const responseData = response.data.data;
      setDetails(responseData);
      //   console.log(details,'Agent model details')
        ...
        {loading ? (
          ...
          <>
            ...
            <DetailsContainer>
              ...
              <Look1><span> <StarOutlined />{" "}Model_ID:{" "}</span>{details?.attributes?.agentModel_ID || "Model ID Does Not Exists"}</Look1>
...
export default AgentModelDetails;
