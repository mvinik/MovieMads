...
const EditSinger = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/api/Singers/${id}?populate=*`
      );
  ...
  const handleFieldChange = async () => {
    try {
      const res = await axios.put(`${API_URL}/api/Singers/${id}`, {
        data: {
          Name: fullName,
  ...
  const handleImagesUpload = async (img) => {
    try {
      ...
      if (imageId) {
        ...
        newImageData.append('refId', localStorage.getItem('SingerId'));
        newImageData.append('ref', 'api::singer.singer');
        // Upload new image
  ...
  const handleVideoFileUpload = async (video) => {
    ...
    try {
      ...
      const newFileData = {
        alternativeText: details?.attributes?.Name || "Singer Video",
        caption: 'Singer Video File',
      };
      formData.append('fileInfo', JSON.stringify(newFileData));
      ...
      // If adding new video
        ...
        formData.append('ref', 'api::singer.singer');
    ...
    return (
      <div key={index} className="gallery-item">
        {image ? (
          ...
          <div className="gallery-item">
            <img src={`https://api.moviemads.com/uploads/empty_image_dd70f20899.jpg`} alt={`Singer `} className="gallery-image" />
             <button className="edit-Pic" onClick={() => { setIsModalOpen('Images'); }}>
              Change Image
            </button>
          </div>
