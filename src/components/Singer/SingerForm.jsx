...
import { Steps, Form, Input, ConfigProvider, Button, Divider, Rate, Upload, Progress, Select, notification, Space } from 'antd';
...
console.log('finalPrice:', finalPrice);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      const response = await axios.post(`${API_URL}/api/singers`, {
        data: {
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < imageFormDatas.length; i++) {
      ...
      imageFormData.append('refId', localStorage.getItem("SingerId"))
      imageFormData.append('ref', 'api::singer.singer')


      if (i == 0) {
        imageFormData.append('field', "Poster")
        const newFileData = {
          alternativeText: localStorage.getItem("SingerId"),
          caption: 'Poster',
        };
    ...
    videoFormData.append('refId', localStorage.getItem("SingerId"))
    videoFormData.append('ref', 'api::singer.singer')
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (Paymentresponse) {
          ...
          try {
            const paymentResponse = await axios.post(`${API_URL}/api/singers/${localStorage.getItem('SingerId')}/${Paymentresponse.razorpay_payment_id}/payment`, {}, option1);
  ...
  return (
    <>
      ...
      <div className="container">
        <div>
          <h1 className='contest-heading'>Singer form <p style={{ fontSize: '1.5rem', padding: '0', margin: '0' }}>(Entry fee of <p className='strikeOut'>Rs.999</p> Now Rs.{discount > 0 ? (<>{finalPrice}</>) : (<>{price}</>)} only)</p> </h1>
        </div>
